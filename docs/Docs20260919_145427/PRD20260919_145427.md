# PRD: Enxugamento e Imersão de Last Horizon

## 1. Resumo Executivo

A feature implementa integralmente a especificação de enxugamento e imersão do jogo 2D Last Horizon. Ela organiza a informação apresentada ao jogador durante a viagem, torna o próximo objetivo compreensível, explicita benefício, custo e consequência antes dos compromissos e dá continuidade às falas dos quatro sobreviventes. O ciclo principal permanece o mesmo: uma viagem de dez dias em uma nave, com exploração física, ordens preventivas, incidentes, coleta e entrega de objetos, socorro, consumo de recursos e desfechos já existentes.

A integração acontece dentro do sketch atual em Processing, por meio dos catálogos, estados globais, tabelas de portais, funções de atualização e camadas modais já presentes. A feature adiciona catálogo editorial ligado aos 22 IDs mecânicos, memória contextual mínima, HUD prioritário, painéis progressivos, previsão noturna verificável e mapa consultivo orientado à próxima passagem. A refatoração segue T01 a T10 e a ordem E0 a E6, preservando balanceamento, regras de quests, controles, fallback de assets, harness e modelo numérico independente.

## 2. Contexto do Projeto

Last Horizon é um jogo 2D de gerenciamento de recursos e exploração em uma nave, desenvolvido em Processing 4.5.6. O sketch usa abas `.pde` combinadas pelo Processing em um único programa, renderiza em 1280 por 720 com grade lógica 640 por 360, ampliação inteira e letterbox, e usa Segoe UI com pixel art sem interpolação. Assets PNG, JSON, Aseprite e WAV ficam em `last_horizon/data/`.

A arquitetura atual mantém o estado da partida em variáveis globais agrupadas por domínio e separa atualização de estado e renderização. O jogo usa tabelas e funções de consulta para portais, escadas, pontos de interação, ordens e incidentes, com revalidação de proximidade, etapa e disponibilidade antes das ações. A camada de assets oferece fallback geométrico para preservar a execução quando uma imagem, spritesheet ou som não está disponível. O protótipo numérico em `prototype/balance-model.mjs` permanece independente e não há banco de dados, backend, autenticação, autorização, `package.json` ou dependências de aplicação.

As áreas de integração são:

| Área | Papel na feature |
| --- | --- |
| `last_horizon/last_horizon.pde` e `game.pde` | Estado global, ciclo diário, atualização, processamento noturno, desfechos e condições de término |
| `last_horizon/tasks.pde` | IDs, catálogos mecânicos, responsáveis, objetos, custos, recompensas, estados de quest e ações |
| `last_horizon/editorial.pde` | Títulos, motivações, vozes, variantes de ambientação e memória contextual |
| `last_horizon/hud.pde`, `ui.pde` e `screens.pde` | Objetivo prioritário, alertas, painéis, modais, rodapé, tipografia, input e hit-test |
| `last_horizon/navigation.pde` e `ship.pde` | Mapa consultivo, salas, conveses, portas, escadas, pontos físicos, chegada e retorno |
| `last_horizon/assets.pde` e `audio.pde` | Sprites, halos, ícones, pisos, fallbacks e reprodução de áudio |
| `last_horizon/test_mode.pde`, `capture.pde` e `tools/snapshot-entrega.mjs` | Modo de desenvolvimento, harness opcional e geração da cópia de entrega |
| `mechanics/`, `interface/`, `events/`, `characters/`, `docs/adr/` | Contratos de regras, fluxo, HUD, salas, eventos, personagens e decisões arquiteturais |

A implementação deve manter a ordem do processamento noturno, a viagem de dez dias, os incidentes nos dias 2, 4, 6, 8 e 10, os sete tipos de incidente com cinco sorteados por partida, os recursos e custos atuais, o limite de uma conclusão por dia, um objeto carregado por vez, os controles `E`, `ENTER` e `ESC`, o Comando como hub e o mapa como consulta. A ausência de asset ou som continua produzindo fallback funcional ou silêncio.

## 3. Personas

| Persona | Descrição | Objetivos | Frustrações |
| --- | --- | --- | --- |
| Ana, jogadora iniciante | Está aprendendo o ciclo de exploração, ordens e recursos de Last Horizon. | Entender o objetivo da viagem, saber qual é o próximo passo e distinguir escolha, aceite, coleta e entrega. | Abrir vários painéis para descobrir uma ação, aceitar uma ordem sem entender a consequência ou perder-se entre salas e conveses. |
| Bruno, jogador estratégico | Conhece jogos de gerenciamento e toma decisões comparando custo, prazo, risco e benefício. | Escolher preventivas e soluções com informação suficiente, prever a noite e decidir sobre socorro e retomadas. | Descobrir custos apenas no momento da entrega, receber previsões que divergem do processamento real ou perder o prazo de um problema. |
| Clara, jogadora interessada na tripulação | Explora a nave e conversa com os sobreviventes para acompanhar suas personalidades durante a viagem. | Reconhecer vozes distintas, entender quando uma fala responde a ajuda, falha, omissão ou risco e conversar sem criar obrigações. | Ouvir frases genéricas, diálogos que afirmam eventos inexistentes ou perder continuidade entre os dias. |
| Diego, mantenedor do projeto | Trabalha no sketch procedural e precisa evoluir a interface sem romper regras, assets, testes ou a cópia de entrega. | Localizar responsabilidades, reutilizar helpers, preservar equivalência mecânica, medir desempenho e compilar com módulos opcionais. | Duplicar lógica de UI, divergir entre desenho e hit-test, introduzir dependências de aplicação ou alterar inadvertidamente o balanceamento. |
...
| Indicador | Meta ou critério | Evidência |
| --- | --- | --- |
| Reconhecimento do próximo passo | Pelo menos 2 de 3 participantes identificam a próxima passagem em até 10 segundos nos percursos Depósito para Dormitório via Comando e entre conveses. | Playtest sem explicação externa |
| Aviso de risco fatal | 3 de 3 participantes reconhecem o aviso fatal antes de confirmar o sono. | Playtest com participantes que não conhecem o código |
| Distinção entre seleção e aceite | Pelo menos 2 de 3 participantes distinguem a seleção de uma preventiva do aceite presencial e explicam o custo ou consequência relevante. | Tarefa observada no playtest |
| Clareza e vontade de continuar | Pelo menos 2 de 3 participantes dão nota mínima 4 para clareza e vontade de continuar. | Escala aplicada no playtest |
| Reconhecimento de vozes | Pelo menos 2 de 3 participantes reconhecem que os quatro sobreviventes têm vozes diferentes. | Playtest de conversas nas três fases da viagem |
| Cobertura editorial | Os 22 IDs mecânicos possuem textos editoriais válidos, responsáveis e pontos existentes; falas-base têm até 160 caracteres e uma ou duas frases. | Harness e inspeção do catálogo |
| Fidelidade da memória | Não há fala de reconhecimento para evento não registrado, o desenho não registra memória e a nova partida zera a memória editorial. | Fixtures de conclusão, falha, omissão, socorro, risco e reinício |
| Equivalência mecânica | O ciclo mantém dez dias, calendário de incidentes, custos, recompensas, prazos, socorro, retomadas, desfechos, portas, escadas e condições de término. | Harness, campanhas de regressão e modelo numérico independente |
| Previsão noturna | A previsão de um estado reproduz o resultado observável de uma noite real iniciada no mesmo estado, sem alterar estado ao ser consultada. | Comparação entre preview e processamento real |
| Desempenho do HUD e cenário | Não há regressão consistente superior a 10% no p95 do tempo por quadro. A medição registra mediana, p95, tempo de carga, memória adicional e preparações em três amostras de 30 segundos. | Medição antes e depois em condições iguais |
| Preparação de recursos e pisos | Os seis ícones disponíveis são preparados em cache e os três conveses que usam o mesmo tile compartilham uma faixa imutável. | Contadores de preparação e verificação de identidade das referências |
| Compatibilidade de entrega | O sketch compila e executa com os módulos de desenvolvimento e também na cópia sem harness e sem modo manual; o modelo independente passa sintaxe e simulação. | Compilação, snapshot e execução das fixtures, incluindo as 2.520 sequências de reserva de peças |
| Legibilidade | Textos, acentos, retratos, custos, botões e destinos permanecem dentro da área útil em 720p e em janela ampliada, sem depender apenas de cor. | Capturas visuais e inspeção em 720p |

## 8. Escopo Negativo

Fazem parte desta entrega:

- T01 a T10 da SPEC, com equivalência observável antes das mudanças de apresentação.
...
- HUD de duas linhas prioritárias, painéis progressivos, previsão noturna verificável e mapa de orientação.
- Validação visual, medição de desempenho, regressão do ciclo e playtest de clareza e diversão.

Não fazem parte desta entrega:

- Alterar custos, recompensas, dificuldade, duração, sorteio, quantidade de quests, rotas físicas ou condições de término.
- Adicionar salas, combate, crafting, inventário livre, romance, reputação, árvores de diálogo, minijogos, dublagem ou cutscenes obrigatórias.
- Adicionar engine, framework, biblioteca de UI, classes de entidades, backend, banco de dados, autenticação ou dependências de aplicação.
- Remover o fallback geométrico, a compatibilidade Aseprite e LPC, o harness de verificação ou o modelo numérico independente.
...
- O sketch Processing 4.5.6, suas abas `.pde` e a ordem atual de atualização e desenho são a base da implementação.
- Os catálogos e regras em `last_horizon/tasks.pde`, `game.pde`, `mechanics/ACTIONS.md` e nos documentos de eventos devem continuar sendo a fonte dos IDs, estados, custos, prazos, recompensas e consequências.
- A integração visual depende das funções e tabelas existentes em `ui.pde`, `hud.pde`, `screens.pde`, `navigation.pde`, `ship.pde` e `assets.pde`, incluindo fallbacks de arte e som.
- A validação depende do harness, das fixtures, de `code/VERIFICATION.md`, de `code/SKETCH_ARCHITECTURE.md`, dos documentos de `interface/`, das personagens, do histórico e das ADRs.
- O modo de teste e a captura devem permanecer opcionais. O fluxo de `tools/snapshot-entrega.mjs` deve gerar a cópia sem os módulos opcionais.
- O modelo `prototype/balance-model.mjs` deve continuar independente do algoritmo Processing e servir como referência numérica.
- A execução deve seguir E0 a E6, começando pela reconciliação documental e pelo baseline e terminando com playtest, medições e documentação sincronizada.
...
Não haverá backend, API HTTP, endpoints, controllers, services externos,
middleware, autenticação, banco de dados ou comunicação externa para esta
feature. O projeto permanece em Processing 4.5.6, como um sketch local formado
por abas `.pde`, com estado global em memória e funções procedurais organizadas
por domínio.

#### Rotas, services e middleware

Não existem rotas, controllers, services ou middleware no projeto e nenhum
desses componentes será criado para esta feature. Os fluxos continuam usando o
estado em memória e as funções já organizadas em `tasks.pde`, `game.pde`,
`editorial.pde` e `navigation.pde`, conforme a responsabilidade existente de
cada módulo.

Não haverá contrato de request ou response, camada de autenticação, validação
de entrada HTTP, tratamento de erros de API, integração externa ou dependência
de servidor. Também não haverá alteração de framework, introdução de biblioteca
de aplicação ou criação de uma camada de serviço artificial para substituir os
patterns procedurais atuais.

#### Escopo da decisão

Todas as decisões desta feature devem preservar o processamento local e em
memória. Nenhuma persistência será criada nesta entrega.

### Frontend

#### Decisão consolidada

A interface continuará sendo um sketch local em Processing 4.5.6, composto por
abas `.pde` procedurais. A renderização continuará usando `PGraphics`, grade
lógica de 640 por 360, saída de 1280 por 720 e letterbox. Não serão adicionados
framework web, biblioteca de UI, CSS, Tailwind, CSS Modules, styled-components,
componentes web, páginas, hooks ou stores convencionais.

A apresentação seguirá a tipografia Segoe UI, a paleta baseada nas constantes
`COL_*`, os fallbacks geométricos para assets ausentes e os helpers existentes
para desenho e interação. Os nomes e responsabilidades atuais das abas são a
fonte da organização da mudança.

#### Organização da UI

A organização escolhida é estender as abas existentes, sem criar uma nova aba
procedural para substituir os módulos atuais. A implementação deve distribuir
as funções conforme estes domínios:

- `screens.pde`: vinheta, estados de vitória e derrota, pausa, prioridade,
  resultados e fechamento de modais.
- `hud.pde`: recursos, objetivo, alerta prioritário e faixa inferior de duas
  linhas.
- `ui.pde`: tipografia, painéis, botões, rodapé de modal, texto quebrado,
  diálogos, ajuda, técnico, sono e hit-test.
- `tasks.pde`: cartões de ordens, coleta, entrega, socorro e retomadas.
- `navigation.pde` e `ship.pde`: alvo, rota, portais, escadas, instruções,
  overlay do mapa e detalhes dos conveses.
- `editorial.pde` e `game.pde`: vozes, ambientação, memória contextual,
  transmissão, incidentes, encerramento do dia e consequências visuais.

Os patterns de composição permanecem nas funções `draw*`; a abertura de
consultas e painéis permanece nas funções `open*`; consultas derivadas devem
ser puras e os controles continuam registrados por `addButton`. O desenho, o
teclado, o mouse, o cursor e o hit-test devem respeitar a camada ativa definida
por `uiLayer` e pelos flags existentes.

#### Ordens e informação progressiva

O painel de ordens exibirá dois cartões lado a lado. Os cartões devem permitir
comparar as ofertas no mesmo espaço e abrir detalhes progressivos no próprio
painel, sem criar uma nova página ou uma tela paralela. A interação seguirá os
botões e o rodapé de modal já existentes, com textos de ação coerentes com os
fluxos atuais de aceitar, voltar e continuar.

#### Mapa consultivo

O mapa continuará como overlay consultivo no fluxo atual. A primeira consulta
exibirá as quatro salas existentes, a posição atual, o destino e a rota
destacada. O detalhe do convés permanecerá disponível no mesmo overlay, sem
abrir uma segunda tela de navegação.

O desenho deve reutilizar as consultas e funções existentes, incluindo
`mapTargetPoint` e `calculateMapRoute`, além das informações de portais e
escadas já mantidas pelos módulos de navegação. Consultar o mapa não pode
mover o técnico, abrir portas, aceitar ordens, coletar, entregar, dormir,
consumir recursos ou alterar o RNG.

#### Camadas, modais e consultas de estado

A camada ativa será resolvida por prioridade usando os flags e o `uiLayer`
existentes. Não será criada uma pilha procedural nova de modais. Cada camada
deve centralizar desenho, entrada, cursor e hit-test para impedir que controles
de uma consulta atravessem para o cenário ou para outro painel.

O estado continuará global e procedural, separado por domínio nas abas atuais.
Não haverá hooks nem stores novos. Consultas puras existentes, como
`currentObjectiveLine`, `mapTargetPoint` e `calculateMapRoute`, continuarão
derivando textos e alvos sem efeitos colaterais. Quando necessário para a
apresentação das ordens e bloqueios, será adicionada uma consulta procedural
de razão de bloqueio no módulo de domínio correspondente, sem duplicar estado
ou alterar as regras mecânicas.

#### Consistência visual e interação

Os painéis, botões, rodapés, diálogos contextuais, HUD e mapa devem usar os
helpers compartilhados de `ui.pde`, manter alinhamentos e sombras da tipografia
comum e permanecer legíveis em 720p e em janela ampliada. A implementação deve
preservar a ordem atual de atualização e desenho, a escala lógica e os
fallbacks de arte e som.

As mudanças de Frontend ficam limitadas à apresentação, às consultas derivadas
necessárias para exibição e ao roteamento visual das camadas. Custos,
recompensas, prazos, sorteios, rotas físicas, condições de término e demais
regras de gameplay permanecem nos módulos existentes e não serão redefinidos
por esta decisão.
...
O projeto mantém o modelo local, single-player e sem identidade do runtime
existente. Não haverá autenticação, sessão, JWT, token, identidade persistente,
backend, endpoint, comunicação externa ou analytics. Também não serão criadas
roles, RBAC, ACL, RLS, policies de banco ou uma camada adicional de permissões.

Não haverá persistência de nomes ou pontuações nesta feature. O gameplay
permanece somente em memória e não haverá banco de dados.

#### Autorizações e validações

As únicas autorizações são as validações de domínio já usadas pelo jogo, sempre
reavaliadas no momento da ação. Elas devem continuar verificando proximidade,
etapa da quest, disponibilidade do objetivo ou NPC, custo, limite diário e
estado do NPC. As ações de aceitar, coletar, entregar e resgatar devem rejeitar
entradas inválidas nas funções de domínio, independentemente do estado visual
do HUD, do teclado ou do mouse.

Não será criada uma abstração de capacidades ou permissões locais. O estado
single-player não exige distinção entre usuários, papéis ou níveis de acesso.

#### Isolamento do modo de teste

O modo de teste e a captura continuam disponíveis somente no ambiente de
desenvolvimento. O fluxo `tools/snapshot-entrega.mjs` deve excluir
`capture.pde` e `test_mode.pde` do artefato final. Hooks necessários para a
compilação devem permanecer inertes e não podem expor Ctrl + K, teleporte,
destaque forçado, captura ou qualquer outra capacidade privilegiada na entrega.

As funções de domínio não podem aceitar uma ação apenas porque um hook de teste
ou um controle visual a solicitou. Mesmo no sketch de desenvolvimento, as
regras de domínio continuam sendo a barreira de autorização das ações do jogo.

#### Middlewares e superfície de execução

Não há middleware HTTP no projeto e nenhum será criado. CORS, CSRF, Helmet,
rate limit, sessão e validação de request não se aplicam ao runtime local. A
superfície de segurança afetada pela feature fica limitada ao isolamento do
artefato de entrega e à revalidação das regras de domínio.
