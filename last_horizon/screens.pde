...
boolean uiActionAllowed(int action){
  int layer = uiLayer();

  if (action == ACTION_CLOSE_MODAL) return layer != LAYER_SCENE;
  if (action == ACTION_OPEN_MAP || action == ACTION_OPEN_ORDERS
    || action == ACTION_OPEN_HELP || action == ACTION_OPEN_RESCUE){
    return layer == LAYER_SCENE && isRoomScreen();
  }
  if (action == ACTION_ORDER_A || action == ACTION_ORDER_B
    || action == ACTION_NEXT_RETRY || action == ACTION_RETRY_QUEST
    || action == ACTION_TOGGLE_ORDER_DETAILS){
    return layer == LAYER_ORDERS;
  }
  if (action == ACTION_CONFIRM_QUEST){
    return layer == LAYER_DIALOGUE || layer == LAYER_TECHNICAL
      || layer == LAYER_ORDERS;
  }
  if (action == ACTION_EVENT_A || action == ACTION_EVENT_B
    || action == ACTION_ACCEPT_SOLUTION || action == ACTION_BACK_SOLUTION){
    return layer == LAYER_EVENT;
  }
  if (action == ACTION_END_DAY) return layer == LAYER_SLEEP;
  if (action == ACTION_RESUME || action == ACTION_RESTART
    || action == ACTION_MAIN_MENU){
    return layer == LAYER_PAUSE;
  }
  if (action == ACTION_START_GAME || action == ACTION_QUIT_GAME)
    return layer == LAYER_SCENE && screen == SCREEN_INIT;
  if (action == ACTION_VIGNETTE_NEXT)
    return layer == LAYER_SCENE && screen == SCREEN_VIGNETTE;
  if (action == ACTION_NEW_GAME)
    return layer == LAYER_SCENE && (screen == SCREEN_VICTORY || screen == SCREEN_GAME_OVER);
...
void doAction(int action){
  if (!uiActionAllowed(action)) return;
