/* Conteúdo editorial separado do catálogo mecânico. IDs, custos, pontos e
   recursos continuam em tasks.pde; esta aba resolve linguagem e memória de
   apresentação. */
...
/* Exactly one editorial row for each ID in tasks.pde. The first field is the
   only technical field; all other fields are player-facing prose. */
...
/* Duas variantes por pessoa em cada uma das três fases da viagem. */
