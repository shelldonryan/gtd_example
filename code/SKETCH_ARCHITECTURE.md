...
partir de um snapshot. `processNight()` usa a mesma função e aplica uma única
projeção ao estado real após a confirmação de sono.
...
runner Windows trabalha sobre cópias temporárias dentro de
`last_horizon/output/` e remove os artefatos ao terminar. Comandos e limites
