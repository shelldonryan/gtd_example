...
NightProjection processNight(){
  ...
  NightProjection night_projection = night_preview;
  if (!applyNightProjection(night_projection)){
    invalidateNightPreview();
    system_message = "A PREVISÃO DA NOITE FICOU OBSOLETA. ABRA O PAINEL NOVAMENTE.";
    return null;
  }
...
void endDay(){
  ...
  if (night_projection == null) return;
